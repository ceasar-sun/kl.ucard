...
if($uid != 0){
  $userRole = $DB->get_record('role_assignments', array('userid'=>$uid));
  $tName = explode("/", $USER->username);
  $nString = $tName[count($tName)-1];
  $firstChar = $nString[0];

  error_log(' -- 1.user id:' . print_r($uid, true) . ' char: ' . print_r($firstChar, true) . ' role: ' . print_r($userRole, true));
